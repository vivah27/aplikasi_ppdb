...
<div style="max-width: 100%;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; flex-wrap: wrap; gap: 15px;">
        ...
        <a href="<?php echo e(route('admin.peran.create')); ?>" style="padding: 12px 24px; background: linear-gradient(135deg, #4f46e5 0%, #6366f1 100%); color: white; text-decoration: none; border-radius: 8px; font-weight: 600; display: inline-flex; align-items: center; gap: 10px; transition: all 0.3s;">
    ...
    <div style="background: white; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); overflow: hidden;">
        <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $peran; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr style="border-bottom: 1px solid #f3f4f6; transition: all 0.3s;" onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                            <td style="padding: 15px; color: #1f2937;"><?php echo e($loop->iteration + ($peran->currentPage() - 1) * $peran->perPage()); ?></td>
                            ...
                            <td style="padding: 15px;">
                                <span style="display: inline-block; padding: 8px 12px; border-radius: 6px; font-size: 12px; font-weight: 600; background: rgba(14, 165, 233, 0.15); color: #0ea5e9;">
                                    <?php echo e($item->pengguna()->count()); ?> pengguna
                                </span>
                            </td>
                            <td style="padding: 15px; color: #6b7280; font-size: 14px;"><?php echo e($item->created_at->format('d M Y')); ?></td>
                            <td style="padding: 15px;">
                                <div style="display: flex; gap: 8px;">
                                    <a href="<?php echo e(route('admin.peran.show', $item->id)); ?>" style="padding: 8px 12px; background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); color: white; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: 600; display: inline-flex; align-items: center; gap: 6px; transition: all 0.3s;" title="Lihat">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="<?php echo e(route('admin.peran.edit', $item->id)); ?>" style="padding: 8px 12px; background: linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%); color: white; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: 600; display: inline-flex; align-items: center; gap: 6px; transition: all 0.3s;" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="<?php echo e(route('admin.peran.destroy', $item->id)); ?>" method="POST" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus?')">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
                                        <button type="submit" style="padding: 8px 12px; background: linear-gradient(135deg, #ef4444 0%, #f87171 100%); color: white; border: none; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; transition: all 0.3s;" title="Hapus">
        ...
        <div style="padding: 20px; border-top: 1px solid #e5e7eb;">
            <?php echo e($peran->links('pagination::bootstrap-4')); ?>
...
<?php echo $__env->make('layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\aplikasi_ppdb\resources\views/admin/peran/index.blade.php ENDPATH**/ ?>