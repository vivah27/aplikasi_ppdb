            ...
            <form action="{{ route('admin.jurusan.update', $jurusan) }}" method="POST">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="kode_jurusan" id="kode_jurusan" class="form-control @error('kode_jurusan') is-invalid @enderror"
                               placeholder="Contoh: RPL, TSM" value="{{ old('kode_jurusan', $jurusan->kode_jurusan) }}" required>
                        @error('kode_jurusan')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nama_jurusan" id="nama_jurusan" class="form-control @error('nama_jurusan') is-invalid @enderror"
                               placeholder="Contoh: Rekayasa Perangkat Lunak" value="{{ old('nama_jurusan', $jurusan->nama_jurusan) }}" required>
                        @error('nama_jurusan')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="number" name="kuota" id="kuota" class="form-control @error('kuota') is-invalid @enderror"
                               placeholder="Contoh: 40" value="{{ old('kuota', $jurusan->kuota) }}" min="1" required>
                        @error('kuota')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-check mt-4">
                            <input type="checkbox" name="is_active" id="is_active" class="form-check-input" 
                                   value="1" {{ old('is_active', $jurusan->is_active) ? 'checked' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" id="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror"
                              placeholder="Deskripsi jurusan" rows="4">{{ old('deskripsi', $jurusan->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
