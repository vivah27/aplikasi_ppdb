...
<div class="container-fluid px-4">
    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <p>
                                ...
                                @endphp
                                <span class="badge bg-{{ $statusColor }}" style="font-size: 0.9rem; padding: 0.5rem 0.75rem;">
                                    {{ $pembayaran->status }}
                                </span>
                            </p>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p>{{ $pembayaran->created_at->format('d F Y \p\u\k\u\l H:i') }}</p>
                        </div>
                        <div class="col-md-6">
                            <p class="text-muted mb-1"><small>Diperbarui</small></p>
                            <p>{{ $pembayaran->updated_at->format('d F Y \p\u\k\u\l H:i') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-4">
            ...
            @else
                <div class="card shadow-sm mb-3 {{ $pembayaran->status === 'Terverifikasi' ? 'border-success' : 'border-danger' }}">
                    <div class="card-header {{ $pembayaran->status === 'Terverifikasi' ? 'bg-success bg-opacity-10' : 'bg-danger bg-opacity-10' }}">
                        <h5 class="mb-0 {{ $pembayaran->status === 'Terverifikasi' ? 'text-success' : 'text-danger' }}">
                            <i class="ti {{ $pembayaran->status === 'Terverifikasi' ? 'ti-check' : 'ti-x' }} me-2"></i>
                            {{ $pembayaran->status === 'Terverifikasi' ? 'Sudah Terverifikasi' : 'Telah Ditolak' }}
                        </h5>
                    </div>
                    <div class="card-body">
                        <div class="alert {{ $pembayaran->status === 'Terverifikasi' ? 'alert-success' : 'alert-danger' }} mb-0">
                            Pembayaran ini sudah di-{{ strtolower($pembayaran->status) }}.
                            <br><small>Diperbarui: {{ $pembayaran->updated_at->format('d M Y H:i') }}</small>
                        </div>
                    </div>
                </div>
            @endif
                ...
                <div class="card-body">
                    ...
                    <div class="list-group list-group-flush">
                        <div class="list-group-item d-flex justify-content-between px-0">
                            ...
                            <strong>{{ $pembayaran->id }}</strong>
                        </div>
                        <div class="list-group-item d-flex justify-content-between px-0">
                            <span class="text-muted"><small>Metode</small></span>
                            <strong>{{ $pembayaran->metode }}</strong>
                        </div>
                        <div class="list-group-item d-flex justify-content-between px-0">
