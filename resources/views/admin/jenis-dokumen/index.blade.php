...
<div style="max-width: 100%;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; flex-wrap: wrap; gap: 15px;">
        ...
        <a href="{{ route('admin.jenis-dokumen.create') }}" style="padding: 12px 24px; background: linear-gradient(135deg, #4f46e5 0%, #6366f1 100%); color: white; text-decoration: none; border-radius: 8px; font-weight: 600; display: inline-flex; align-items: center; gap: 10px; transition: all 0.3s;">
    ...
    <div style="background: white; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); overflow: hidden;">
                ...
                <tbody>
                    @forelse ($jenisDokumen as $item)
                        <tr style="border-bottom: 1px solid #f3f4f6; transition: all 0.3s;" onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                            <td style="padding: 15px; color: #1f2937;">{{ $loop->iteration + ($jenisDokumen->currentPage() - 1) * $jenisDokumen->perPage() }}</td>
                            ...
                            <td style="padding: 15px;">
                                <span style="display: inline-block; padding: 6px 12px; border-radius: 6px; font-size: 12px; font-weight: 600; background: {{ $item->wajib ? 'rgba(239, 68, 68, 0.15)' : 'rgba(16, 185, 129, 0.15)' }}; color: {{ $item->wajib ? '#ef4444' : '#10b981' }}">
        ...
        <div style="padding: 20px; border-top: 1px solid #e5e7eb;">
            {{ $jenisDokumen->links('pagination::bootstrap-4') }}
        </div>
    </div>
</div>
